...
不同的開發模式，會有安全性的問題，會採用把檔案下載到本地端，比較安全，別人無法更改這些檔案。

popper.min.js 為第三方的東西
...
Layout 裡面的 grid 是 bootstrap 採用 flex 做出來的格線系統，會稱為系統是因為有規則存在。

格線系統是將**空間**分割成 12 欄，分割成**12 等分**。

`.container`，是一個定寬容器。(有固定的寬度，並且在不同的裝置會有不同的寬度)

`.container-fluid`，沒有固定寬度。
...
直接使用五個`col`即可，因為格線系統會自動分欄，要幾欄有幾欄，寬度它會幫我決定。
...
格線系統是將空間分割成 12 欄，分割成 12 等分，所以假如要放 3 個 column，就使用 12/4=3，`col-4`。
...
- 576px 最小型的裝置
- 576px 手機的橫向
- 768px 螢幕解析度大於等於 768 就會生效，平板的直向
- 992px 平板的橫向(較舊的裝置)
- 1200px 平板的橫向(ipad pro)
- 1400px Bootstrap5 增加的
...
2. `flex wrap` 格線可以做換列。
3. `margin-left/right`的負值，`container`會友左右的`padding`，把它抵銷掉，可以占滿空間。
...
使用`align-items-center`沒效果時，可能是`.container`與`.row`未設定高度的問題，所以在後面設置`h-100`即可。
...
`order-last`，order 設定一樣時，按照原始碼的順序，A 前 B 後。
